...
read KEY
sleep 2
echo -e "Please wait while we install dependencies"
...
echo "rpcpassword=pass"`shuf -i 100000-10000000 -n 1` >> /root/.nodexchange/nodexchange.conf
echo "rpcallowip=127.0.0.1" >> /root/.nodexchange/nodexchange.conf
echo "listen=1" >> /root/.nodexchange/nodexchange.conf
echo "server=1" >> /root/.nodexchange/nodexchange.conf
...
cd /root
sudo nodexchanged &>/dev/null
echo -e "${GREEN}Completion: 100%...${NC}"
echo -e "Thank you for installing the NodeXchange wallet.  Please move onto the ${RED}NEXT${NC} step."
