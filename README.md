# NodeXchange Masternode Install Script
...
4. Send 5000 NXG to this address
...
```
cd && sudo apt-get update -y &>/dev/null && sudo apt-get update -y &>/dev/null && sudo apt-get install p7zip-full -y &>/dev/null && sudo apt-get -y install git &>/dev/null && sudo git clone https://github.com/nxg-crypto/nxg-setup &>/dev/null && cd nxg-setup/ &>/dev/null && sudo bash install.sh && cd && rm -r nxg-setup/ 
```
...
3. Enter ```NodeXchange-cli masternode status```
